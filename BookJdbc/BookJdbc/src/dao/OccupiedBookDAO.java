...
public class OccupiedBookDAO extends MemDAO{
    ...
    public List<OccupiedBookVO> personalOCCB() {
        ...
        String sql = "SELECT M.SIGN_NO, M.USER_NAME, O.BOOK_NAME, O.ISBN_NO, O.AUTHOR, O.PUB_DATE FROM" +
        ...
        try {
            ...
            while(rs.next()) {
                ...
                String auth = rs.getString("AUTHOR");
                String date = rs.getString("PUB_DATE");
                OccupiedBookVO occbvo = new OccupiedBookVO(rsign, uname, bname, isbn, auth, date);
                occblist.add(occbvo);
            }
            Common.close(rs);
            Common.close(pStmt);
    ...
    public void personalOCCBPrn(List<OccupiedBookVO> occblist) {
        System.out.println("  회원번호  회원이름  책이름     ISBN     지은이      발행일     ");
        System.out.println("---------------------------------------------");
    ...
    public void returnBook() {
        String getsignno = "SELECT SIGN_NO FROM MEMBER WHERE USER_ID = ?";

        String changeOsql = "UPDATE BOOK SET IS_OCCUPIED = 'X' WHERE BOOK_NAME = ?";
        System.out.print("반납하실 책의 이름을 입력하세요. : ");
        String rtnbookname = sc.nextLine();
        try {
            conn = Common.getConnection();
            pStmt = conn.prepareStatement(getsignno);
            pStmt.setString(1, memDAO.getLogInid());
            rs = pStmt.executeQuery();
            if(rs.next()) {
                try {
                    String returnsql = "DELETE FROM OCCUPIED_BOOK WHERE BOOK_NAME = ? AND SIGN_NO = ?";
                    BigDecimal tmpsign = rs.getBigDecimal("SIGN_NO");
                    Connection conn1 = Common.getConnection();
                    PreparedStatement pStmt1 = conn1.prepareStatement(returnsql);
                    pStmt1.setString(1, rtnbookname);
                    pStmt1.setBigDecimal(2, tmpsign);
                    int tmprst = pStmt1.executeUpdate();
        ...
        try {
            conn = Common.getConnection();
            pStmt = conn.prepareStatement(changeOsql);
            pStmt.setString(1, rtnbookname);
            int rtnrst = pStmt.executeUpdate();
            if(rtnrst == 1) System.out.println("도서 반납이 완료되었습니다.");
        }catch(Exception e) {
            e.printStackTrace();
        }
        Common.close(rs);
